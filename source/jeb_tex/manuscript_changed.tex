...
%DIF ADD manuscript_new2.tex   Thu Sep 16 12:45:47 2021
%% V1.0
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \corres{(\email{endlein@gmail.com})}
\DIFaddend 

\date{Received 11 May 2021; revised \DIFdelbegin \DIFdel{11 May }\DIFdelend \DIFaddbegin \DIFadd{16 September }\DIFaddend 2021} %CHECK DATE
...
\DIFdel{)}\DIFdelend , used for precise lateral movements \DIFaddbegin \DIFadd{(step size = 75 nm)}\DIFaddend . Additionally, a separate Z-piezo element \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{(P-629.1CD, }\emph{\DIFadd{Physik Instrumente, Germany}}\DIFadd{, resolution = 3 nm), }\DIFaddend fixed upright,
...
were used for contact area extraction while the side view imaging was used to \DIFdelbegin \DIFdel{visually aid }\DIFdelend \DIFaddbegin \DIFadd{aid in }\DIFaddend orienting the pad
...
to the pad circumference \DIFaddbegin \DIFadd{(``Cassie state")}\DIFaddend .
...
show that the bubble by itself has a negligible capillary contribution \DIFaddbegin \DIFadd{(less than 3\%)
}\DIFaddend to the net underwater adhesion of the pad. Direct force measurement
...
\DIFdelbegin %DIFDELCMD < \ack{We are grateful to Prof. Dr. Eduard Arzt and Dr. Ren\' {e} Hensel (Leibniz Institute for New Materials, Saarbr\"{u}cken, Germany)  for fruitful discussions.} 
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \ack{We are grateful to Prof. Dr. Eduard Arzt and Dr. Ren\' {e} Hensel (Leibniz Institute for New Materials, Saarbr\"{u}cken, Germany)  for fruitful discussions. We further thank the referees for their valuable suggestions in improving the text.} 
\DIFaddend 
